...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>{{ data ? 'Izmeni zvanje' : 'Dodaj zvanje' }}</h2>
    <mat-dialog-content>
...
export class ZvanjeFormComponent implements AfterViewInit {
  data = inject<ZvanjeResponse | null>(MAT_DIALOG_DATA);
  private dialogRef = inject(MatDialogRef<ZvanjeFormComponent>);
  ...
  submit() {
    ...
    const call: Observable<unknown> = this.data
      ? this.service.update(this.data.idZvanje, request)
