...
export class RadnoMestoFormComponent implements AfterViewInit {
  data = inject<RadnoMestoResponse | null>(MAT_DIALOG_DATA);
  private dialogRef = inject(MatDialogRef<RadnoMestoFormComponent>);
  ...
  submit() {
    ...
    const call: Observable<unknown> = this.data
      ? this.service.update(this.data.radnoMestoId, request)
